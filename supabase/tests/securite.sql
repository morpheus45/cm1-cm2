...
select public.depose_seance('aaaaaa', 'Léa', 'Martin', 'CM1', 2::smallint, 'maths', 'questions',
  '[{"domain":"calcul","correct":3,"total":4}]');

select pg_temp.doit_echouer(
  $q$select public.depose_seance('ZZZZZZ','Léa','Martin','CM1',2::smallint,'maths','questions','[]')$q$,
  'code de classe inconnu', 'un code de classe inconnu doit être refusé');

select pg_temp.doit_echouer(
  $q$select public.depose_seance('AAAAAA','Léa','Martin','CM1',2::smallint,'maths','questions','{"x":1}')$q$,
...
select public.depose_seance('AAAAAA', 'lea', 'MARTIN', 'CM1', 2::smallint, 'francais', 'questions', '[]');
select public.depose_seance('AAAAAA', ' Léa ', '  Martin ', 'CM1', 2::smallint, 'francais', 'questions', '[]');
select public.depose_seance('AAAAAA', 'LÉA', 'martin', 'CM1', 2::smallint, 'francais', 'questions', '[]');
-- Une autre Léa.
select public.depose_seance('AAAAAA', 'Léa', 'Dubois', 'CM1', 2::smallint, 'maths', 'questions', '[]');
reset role;
...
select pg_temp.attendu((select count(*) from public.sessions), 5, 'la maîtresse A voit leurs cinq séances');
